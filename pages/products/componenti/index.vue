<template>
    ...
    >
<div role="status" v-for="producta in paginatedProducts" :key="producta.id" class="max-w-sm p-4 border border-gray-200 rounded shadow animate-pulse md:p-6 dark:border-gray-700">
    ...
    <span class="sr-only">Loading...</span>
      ...
      <div v-else class="pt-28 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4">
        <div v-for="producta in paginatedProducts" :key="producta.id" class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
          <div>
            <nuxt-link :to="`/products/componenti/${producta.id}`">
              <img class="p-8 max-w-[15rem] h-[15rem] object-contain mx-auto" :src="producta.image" :alt="producta.title ? producta.title : ''"/>

              <!-- <img class="p-8 max-w-[15rem] h-[15rem] object-contain mx-auto" :src="producta.image" :alt="producta.title ? producta.title : ''"/> -->
            </nuxt-link>
          </div>
          <div class="px-5 pb-5">
            <nuxt-link :to="`/products/componenti/${producta.id}`" :title="producta.title">
              <h5 class="text-xl font-semibold tracking-tight text-gray-900 dark:text-white">
                {{ truncateText(producta.title, 30) }}
              </h5>
            </nuxt-link>
            <p>{{ producta.rank }}</p>
          ...
          <div class="flex items-center justify-between">
            ...
            >
            <a
            target="_blank" :href="producta.AffiateLink + `nuxt-dev-21`"
...
const { pending, data: allProducts } = useFetch(
  "https://eu-central-1.aws.data.mongodb-api.com/app/data-xdnek/endpoint/componenti",
  {
    lazy: false,
    transform: (productas) => {
      return productas.map((producta) => ({
        id: producta._id,
        title: producta.Title,
        image: producta.Thumbnail,
        brand: producta.Brand,
        asin: producta.ASIN,
        price: producta.Price,
        rank: producta.SalesRank,
        AffiateLink: producta.AffiateLink
      }));
    },
  }
);
...
const filteredProducts = computed(() => {
  ...
  return allProducts.value.filter(producta => parseFloat(producta.price) > 0 && producta.rank !== 0);
});
