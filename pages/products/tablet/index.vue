<template>
    ...
    >
<div role="status" v-for="productc in paginatedProducts" :key="productc.id" class="max-w-sm p-4 border border-gray-200 rounded shadow animate-pulse md:p-6 dark:border-gray-700">
    ...
    <span class="sr-only">Loading...</span>
      ...
      <div v-else class="pt-28 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4">
        <div v-for="productc in paginatedProducts" :key="productc.id" class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
          <div>
          ...
          <div class="px-5 pb-5">
            <nuxt-link :to="`/products/tablet/${productc.id}`" :title="productc.title">
...
const filteredProducts = computed(() => {
  ...
  return allProducts.value.filter(productc => parseFloat(productc.price) > 0 && productc.rank !== 0);
});
